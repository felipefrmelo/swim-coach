...
function ValidationPanel({ errors }: { errors: string[] }) { return errors.length ? <section className="validation-panel validation-error"><AlertTriangle className="size-5" /><div><h2 className="font-bold">Ajuste estes blocos</h2>{errors.map((error) => <p className="mt-1 text-sm" key={error}>{error}</p>)}</div></section> : <section className="validation-panel"><CheckCircle2 className="size-5" /><div><h2 className="font-bold">Pronto para salvar</h2><p className="mt-1 text-sm">Todas as distâncias terminam na parede.</p></div></section>; }
