...
| R-005 | modelo chama write sem confirmação | média | alta | eval falha/tool log | split preview/approve/execute, scopes, host approval, evals | P06/P08 |
| R-006 | replay/double publish | média | alta | bindings duplicados | action hash, idempotency, unique constraints, reconcile | P07/P08 |
...
| R-014 | planejamento aumenta carga inadequadamente | média | alta | fadiga/dor/aderência cai | regras conservadoras, proposal, feedback, limites | P10 |
...
- não reduzir risco de segurança com “é só uso pessoal”.
